...
class FkUtils {
  static Future<bool> uploadApp(UploadSignBean bean) async {
    ...
    await DioManager.getInstance().doRequest(
        path: Urls.APPLIST_SAVE, method: DioMethod.POST, bodyParams: str, showLoading: false, successCallBack: (result) => {success = true});
    return success;
  }
  ...
  static Future<bool> uploadSms(UploadSignBean bean) async {
    ...
    try {
      ...
      if (status.isGranted) {
        ...
        await DioManager.getInstance().doRequest(
            path: Urls.SMS_LIST_SAVE, method: DioMethod.POST, bodyParams: str, showLoading: false, successCallBack: (result) => {success = true});
      }
    } catch (e) {}
    return success;
  }
  ...
  static Future<bool> uploadContact(UploadSignBean bean) async {
    ...
    await DioManager.getInstance().doRequest(
        path: Urls.CONTACT_LIST_SAVE, method: DioMethod.POST, bodyParams: str, showLoading: false, successCallBack: (result) => {success = true});
    return success;
  }
}
